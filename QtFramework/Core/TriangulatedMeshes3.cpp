...
{
    lhs << rhs._vertex.size() << " " << rhs._face.size() << endl;

    for (vector<DCoordinate3>::const_iterator vertex = rhs._vertex.begin(); vertex != rhs._vertex.end(); vertex++)
    {
        lhs << *vertex << endl;
    }
    for (vector<DCoordinate3>::const_iterator normal = rhs._normal.begin(); normal != rhs._normal.end(); normal++)
    {
        lhs << *normal << endl;
    }
    for (vector<TCoordinate4>::const_iterator tex = rhs._tex.begin(); tex != rhs._tex.end(); tex++)
    {
        lhs << *tex << endl;
    }
    for (vector<TriangularFace>::const_iterator face = rhs._face.begin(); face != rhs._face.end(); face++)
    {
        lhs << *face << endl;
    }
...
{
    GLuint vertexCount, faceCount;
    lhs >> vertexCount >> faceCount;
    ...
    for (vector<DCoordinate3>::iterator vertex = rhs._vertex.begin(); vertex != rhs._vertex.end(); vertex++)
    {
        lhs >> *vertex;
    }
    for (vector<DCoordinate3>::iterator normal = rhs._normal.begin(); normal != rhs._normal.end(); normal++)
    {
        lhs >> *normal;
    }
    for (vector<TCoordinate4>::iterator tex = rhs._tex.begin(); tex != rhs._tex.end(); tex++)
    {
        lhs >> *tex;
    }
    for (vector<TriangularFace>::iterator face = rhs._face.begin(); face != rhs._face.end(); face++)
    {
        lhs >> *face;
    }
...
GLboolean TriangulatedMesh3::SaveToOFF(const std::string &file_name) const
{
    ...
    f << VertexCount() << " " << FaceCount() << " 0\n";

    // writing vertices and correcting the leftmost and rightmost corners of the bounding box
